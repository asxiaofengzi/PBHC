...
class MultimodalMotionTrackingEnv(LeggedRobotMotionTracking):
    ...
    def _init_multimodal_components(self):
        ...
        # 融合控制器（延迟初始化，等待obs_dim确定）
        self.fusion_controller = None
        self._need_fusion_controller_init = True
    ...
    def _init_fusion_controller_if_needed(self):
        """在观测维度确定后初始化融合控制器"""
        if self._need_fusion_controller_init and hasattr(self, '_obs_buf_dict') and 'actor_obs' in self._obs_buf_dict:
            # 获取实际的观测维度
            actual_obs_dim = self._obs_buf_dict['actor_obs'].shape[-1]
            
            print(f"[MultimodalEnv] 延迟初始化融合控制器 - obs_dim: {actual_obs_dim}")
            ...
            self._need_fusion_controller_init = False
            print(f"[MultimodalEnv] 融合控制器初始化完成")
    ...
    def _compute_multimodal_observations(self):
        """计算多模态相关的观测信息"""
        # 确保融合控制器已初始化
        self._init_fusion_controller_if_needed()
